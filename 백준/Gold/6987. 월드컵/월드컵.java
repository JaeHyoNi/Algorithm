...
public class Main {
	...
	public static void dfs(int t1, int t2) {
		...
		if(arr[t1][0]>=1 && arr[t2][2] >=1 && !flag) {
			arr[t1][0]-=1; arr[t2][2]-=1;
			dfs(t1, t2+1);
			arr[t1][0]+=1; arr[t2][2]+=1;
		}
		if(arr[t1][1]>=1 && arr[t2][1] >=1 && !flag) {
			arr[t1][1]-=1; arr[t2][1]-=1;
			dfs(t1, t2+1);
			arr[t1][1]+=1; arr[t2][1]+=1;
		}
		if(arr[t1][2]>=1 && arr[t2][0] >=1 && !flag) {
			arr[t1][2]-=1; arr[t2][0]-=1;
			dfs(t1, t2+1);
			arr[t1][2]+=1; arr[t2][0]+=1;
		}
	}
